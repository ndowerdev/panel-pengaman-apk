...
const emit = defineEmits(['addAsnBlock', 'removeAsnBlock'])
...
<template>
  <div>
    <div class="box bg-white">
      ...
      <InputText
        v-model="$props.toInput.asn"
        label="ASN Number"
        @keydown.enter="$emit('addAsnBlock', $props.toInput.asn)"
      />
      <button
        :class="$props.inputStatus.asn.class"
        :disabled="$props.inputStatus.asn === false"
        type="button"
        class=" text-white font-bold py-1 px-2 rounded w-full"
        @click="$emit('addAsnBlock', $props.toInput.asn)"
      >
        ADD
      </button>
      <div
      ...
      >
        <ul class="flex flex-col divide-y w-full">
          <li
            v-for="(item, index) in formData.asn"
            :key="index"
            class="flex flex-row"
            ...
            <div class="select-none cursor-pointer hover:bg-gray-50 flex flex-1 items-center p-4">
              ...
              <div class="flex flex-row justify-center">
                <button
                  ...
                  @click="$emit('removeAsnBlock', index)"
                >
                  ❎
                </button>
              </div>
